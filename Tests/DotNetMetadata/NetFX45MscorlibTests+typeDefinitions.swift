...
extension NetFX45MscorlibTests {
    @Test func testTypeGenericParamEnumeration() throws {
        ...
        let action1 = try #require(assembly.resolveTypeDefinition(fullName: "System.Action`1"))
        #expect(try action1.genericParams.map({ $0.name }) == [ "T" ])

        // Delegate with 2 generic parameters
        let action2 = try #require(assembly.resolveTypeDefinition(fullName: "System.Action`2"))
