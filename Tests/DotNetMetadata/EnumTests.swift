...
internal final class EnumTests {
    ...
    @Test func testEnumerantNames() throws {
        let enumDefinition = try #require(try assembly.resolveTypeDefinition(fullName: "MyEnum") as? EnumDefinition)
        #expect(enumDefinition.fields.filter { $0.isStatic }.map { $0.name }.sorted() == ["A", "B"])
    }

    @Test func testUnderlyingType() throws {
        #expect(
            #require(try assembly.resolveTypeDefinition(fullName: "MyEnum") as? EnumDefinition).underlyingType.fullName == "System.Int32")
        #expect(
            #require(try assembly.resolveTypeDefinition(fullName: "MyShortEnum") as? EnumDefinition).underlyingType.fullName == "System.Int16")
    }
    ...
    @Test func testEnumerantValues() throws {
        ...
        let shortEnumDefinition = try #require(try assembly.resolveTypeDefinition(fullName: "MyShortEnum") as? EnumDefinition)
        #expect(try #require(#require(shortEnumDefinition.findField(name: "A")).literalValue) == .int16(42))
    }

    @Test func testIsFlags() throws {
        #expect(!#require(try assembly.resolveTypeDefinition(fullName: "MyEnum") as? EnumDefinition).isFlags)
        #expect(#require(try assembly.resolveTypeDefinition(fullName: "MyFlagsEnum") as? EnumDefinition).isFlags)
    }
}